    ...
    where R : IComparable<R>
{
    ...
    /// <summary>
    /// Binary search on the list. See <see cref="CollectionExtensions.BinarySearchNearest{T, R}(IList{T}, R, Func{T, R})"/>.
    /// </summary>
    ...
    public int Search(R value)
    {
        ...
        if (hasResetted || (prevRebisectAvailable && value.CompareTo(prevRebisect) <= 0) || (nextRebisectAvailable && value.CompareTo(nextRebisect) >= 0))
        {
            hasResetted = false;
            cachedIndex = list.BinarySearchNearest(value, property);
            cachedIndex = Math.Clamp(cachedIndex, 0, count - 1);
        ...
        {
            cachedIndex++;
            cachedIndex = Math.Min(cachedIndex, count - 1);
        }

        if (prevDecrementAvailable && value.CompareTo(prevDecrement) < 0)
        {
            cachedIndex--;
            cachedIndex = Math.Max(cachedIndex, 0);
        }
